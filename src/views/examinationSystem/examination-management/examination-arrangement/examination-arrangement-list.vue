...
<div>
  ...
  <el-card :style="style">
      ...
      <el-table-column fixed="right" label="操作" width="310" align="center">
        <template slot-scope="scope">
          <div style="text-align:right;">
            ...
            <span v-if="scope.row.flagExaminationInformation">
              <el-button type="primary" size="mini" @click="viewDistribution(scope.row)">查看考务信息</el-button>
              <el-button v-if="scope.row.examinationStatusName!=='停用'" type="success" size="mini" @click="makeUpDistribution(scope.row)">补生成考务信息</el-button>
            </span>
          </div>
        </template>
      </el-table-column>
    ...
    <!-- 补生成考务 -->
    <exam-student-list :dataForm="editForm" :visible.sync="makeUpStudentsVisible"></exam-student-list>
  </el-card>
</div>
</template>
<script>
...
import examStudentList from './examimation-student-list' // 补生成考务
export default {
  components: {orgTree, pagination, addEdit, paperSelect, studentArrgentInfo, arrgentInfo, examPaper, examStudentList},
  data () {
    return {
      style: '',
      conditionForm: {},
  ...
  methods: {
    ...
    makeUpDistribution (row) {
      ...
      this.editForm = data
      this.makeUpStudentsVisible = true
    },
    // 查看考试安排详情
