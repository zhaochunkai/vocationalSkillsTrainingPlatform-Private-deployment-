...
<div>
  ...
  <el-card :style="style">
    ...
    <el-table class="t-table-list"  ref="multipleTable" stripe size="small" :data="tableData" tooltip-effect="dark" style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="evaluationTypeName" label="评价类型" width="140"></el-table-column>
      <el-table-column prop="trainingProgramsName" label="项目名称" align="center"></el-table-column>
      <el-table-column prop="evaluationPeriodName" label="评价周期" width="260" align="center">
        <template slot-scope="scope">
          {{new Date(scope.row.startDate).toLocaleDateString()}} ~ {{new Date(scope.row.endDate).toLocaleDateString()}}
        </template>
      </el-table-column>
      <el-table-column prop="evaluationPlanUsageStatusName" label="状态" width="70"></el-table-column>
      <el-table-column  fixed="right" label="" width="340" align="right">
  ...
  <!-- 评价统计 -->
  <evaluation-statistics :dataForm="addEditForm" :visible.sync="statisticsVisible"></evaluation-statistics>
  <!-- 评价统计 -->
  <evaluation-info :dataForm="infoForm" :visible.sync="evaluationInfoVisible"></evaluation-info>
</div>
</template>
<script>
import { getEvaluationPlan, deleteEvaluationPlan, updateEvaluationPlanEnable } from '@/server/online-module'
import { pageSizes, pageSize } from '@/config.js'
import pagination from '@/components/pagination'
import addEdit from './evaluating-list-add'
import evaluationStatistics from '../evaluation-statistics/evaluation-statistics'
import evaluationInfo from '../evaluation-infomations/evaluation-infoData'
export default {
  components: {pagination, addEdit, evaluationStatistics, evaluationInfo},
  data () {
    return {
      style: '',
      pageSizes,
  ...
  methods: {
    ...
    delEvaluate  (row) { // 删除
      this.$confirm(`此操作将 删除评价 【${row.evaluationName}】，请确认？`, '提示', {
        confirmButtonText: '通过',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteEvaluationPlan(row).then(() => {
